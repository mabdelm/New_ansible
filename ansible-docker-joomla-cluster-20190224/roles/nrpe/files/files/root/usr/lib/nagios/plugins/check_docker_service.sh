...
if [ "${NUMCONT}" -le  "${CONTCRIT}" ] ;  then
    echo "CRITICAL - ${NUMCONT} replicas running under ${CONTNAME} service"
    exit $ST_CR
elif [ "${NUMCONT}" -le  "${CONTWARN}" ] ;  then
    echo "WARNING - ${NUMCONT} replicas running under ${CONTNAME} service"
    exit $ST_WR
else
    echo "OK - ${NUMCONT} replicas running under ${CONTNAME} service"
    exit $ST_OK
fi